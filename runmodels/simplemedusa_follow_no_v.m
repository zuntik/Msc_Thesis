...
% T = 70s but no v
constants.xi = [constants.r, 0, pi/2, 0.8956, 2*pi/constants.T]; % x y yaw u v
constants.xf = [constants.r, 0, pi/2+2*pi, 0.8956, 2*pi/constants.T ];

% T = 140s and no v
constants.xi = [constants.r, 0, pi/2, 0.4486, 2*pi/constants.T];
constants.xf = [constants.r, 0, pi/2+2*pi, 0.4486, 2*pi/constants.T];
...
constants.statebounds = [
    % x   y    yaw   u   v     r
    -1000 -1000 -1000 0 -.74 % lower state bounds
    1000 1000 1000 1.1 .74 % upper state bounds
    ];
...
constants.desiredpoints=constants.r*[...
    cos(linspace(0,2*pi,constants.N+1)).',...
...
fplot(@(t)calc_tau_r(xOut,constants,t), [0, constants.T]);
...
%     tau_r = @(t) BernsteinEval(X(:,8),constants.T,t);
    ...
    [t,xy] = ode45(@(t,xy)odefunc(t,xy,tau_u,tau_r,constants.MODELPARAMS), [0 constants.T], X(1,1:5));%6));
...
%         dydt(6) = 1/m_r*(tau_r(t) + m_uv*u*v - d_r*r+fr); % dr  
        dydt(1) = u*cos(yaw) + Vcx; %dx
        dydt(2) = u*sin(yaw) + Vcy; % dy
        dydt(3) = r; % dyaw
        dydt(4) = 1/m_u*(tau_u(t) - d_u*u+fu); %du
%         dydt(5) = 1/m_v*(-m_u*u*r+fv); % dv
%         dydt(6) = 1/m_r*(tau_r(t) - d_r*r+fr); % dr
        dydt(5) = 1/m_r*(tau_r(t) - d_r*r+fr); % dr
    end
...
%     ];
    ceq = [
        ...
        DiffMat*y - u.*sin(yaw);
        DiffMat*yaw - r;
        DiffMat*u - 1/m_u*(tau_u - d_u.*u+fu);
        DiffMat*r - 1/m_r*(tau_r - d_r.*r+fr);
    ];
    c = [];
...
%     yaw = constants.desiredpoints(:,3);
    ...
    [u, v, r, tau_u, tau_r] = calcothers([xp, yp, yaw], constants);
...
%     tau_r = m_r*DiffMat*r - m_uv * u.*v + d_r .*r;
    tau_u = m_u*DiffMat*u + d_u.*u;
    tau_r = m_r*DiffMat*r - m_uv + d_r .*r;
end
