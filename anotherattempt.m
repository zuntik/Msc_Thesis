...
Aeq_v = zeros(N+1,(N+1)*(Nx+Nu)+1);
Aeq_v(:,(N+1)*4+1:(N+1)*5 ) = eye(N+1);
beq_v = V*ones(N+1,1);
...
function [c, ceq] = nonlcon(X)
    ...
    [~, ~, r1N, r2N, VN, dXN, dYN, dr1N, dr2N, ~, UN1, ~] = getTrajectories(X);
  
    % differential constraint
    Mag = sqrt(r1N.^2 + r2N.^2);
    r1N = (r1N + eps)./(Mag+eps); r2N = (r2N + eps)./(Mag + eps);

    ceq = [dXN - tf*VN.*r1N;dYN - tf*VN.*r2N; dr1N + tf*r2N.*UN1; dr2N - tf*r1N.*UN1]; 
